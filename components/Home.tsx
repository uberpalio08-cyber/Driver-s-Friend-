  ...
  return (
    <div className="space-y-6 pb-40">
      <header className="flex justify-between items-center pt-4 px-1">
        <div>
          <p className="text-[9px] font-black text-blue-500 uppercase tracking-widest mb-1">Status Ativo</p>
      ...
      <div className="bg-slate-900 p-8 rounded-[2.5rem] border-l-8 border-blue-600 shadow-2xl">
         <p className="text-[10px] font-black uppercase text-slate-500 mb-2 tracking-widest">Lucro Livre (Hoje)</p>
         ...
         <p className="text-[9px] font-black text-blue-400 mt-3 uppercase tracking-widest text-right">{progress.toFixed(0)}% da Meta Batida</p>
      </div>
            ...
            <div className="flex gap-4">
               <button onClick={() => setShowAppConfig(true)} className="flex-1 bg-slate-900 p-5 rounded-2xl flex items-center justify-between border border-white/5">
                  <div className="text-left">
                     <p className="text-[8px] font-black text-slate-500 uppercase mb-1">App Atual</p>
                     <p className="text-xs font-black text-white italic truncate">{selectedApp?.name || '---'}</p>
                  </div>
                  <Settings size={18} className="text-blue-500" />
               </button>
               <button onClick={() => { setOdoInput(''); setShowOdo(true); }} className="bg-rose-900/30 text-rose-500 px-6 rounded-2xl font-black uppercase text-[10px] border border-rose-500/20">Finalizar</button>
            </div>

            {phase === 'ON_SHIFT' && (
              <button onClick={() => setPhase('ACCEPTING')} className="w-full bg-blue-600 text-white py-10 rounded-[3rem] font-black uppercase italic text-xl border-b-8 border-blue-800 shadow-2xl flex items-center justify-center gap-4">
                <UserPlus size={32} /> Aceitar Corrida
              </button>
            )}
            {phase === 'ACCEPTING' && (
              <button onClick={() => setPhase('BOARDING')} className="w-full bg-white text-black py-10 rounded-[3rem] font-black uppercase italic text-xl border-b-8 border-slate-300 shadow-2xl flex items-center justify-center gap-4">
                <Users size={32} /> Iniciar Viagem
              </button>
            )}
            {phase === 'BOARDING' && (
              <button onClick={() => setShowGross(true)} className="w-full bg-emerald-600 text-white py-10 rounded-[3rem] font-black uppercase italic text-xl border-b-8 border-emerald-800 shadow-2xl flex items-center justify-center gap-4">
                <Flag size={32} /> Finalizar & Cobrar
              </button>
            )}
          </div>
        )}
      ...
      {/* MODAL CONFIGURAÇÃO DE APPS - REVISADO VISUALMENTE */}
      {showAppConfig && (
        <div className="fixed inset-0 bg-slate-950/98 backdrop-blur-2xl z-[200] flex items-center justify-center p-6">
          <div className="w-full max-w-sm p-8 bg-slate-900 border border-white/10 rounded-[3rem] shadow-2xl space-y-6 animate-up max-h-[90vh] overflow-y-auto">
            <h2 className="text-2xl font-black italic uppercase text-white tracking-tighter text-center">Configuração</h2>
            ...
            <div className="bg-slate-950 p-6 rounded-[2rem] border border-white/5 space-y-4">
              <p className="text-[10px] font-black text-blue-500 uppercase text-center mb-2 tracking-widest">Novo Perfil de App</p>
              <input className="w-full !bg-slate-900 !text-white !border-slate-800" placeholder="Nome (Ex: Uber Comfort)" value={newApp.name} onChange={e => setNewApp({...newApp, name: e.target.value})} />
              <input className="w-full !bg-slate-900 !text-white !border-slate-800 text-center" type="number" placeholder="Taxa do App (%)" value={newApp.tax} onChange={e => setNewApp({...newApp, tax: e.target.value})} />
              
              <div className="flex bg-slate-900 p-1 rounded-2xl border border-white/5">
                <button onClick={() => setNewApp({...newApp, isFixed: !newApp.isFixed})} className={`flex-1 py-3 rounded-xl font-black text-[10px] uppercase transition-all ${newApp.isFixed ? 'bg-blue-600 text-white' : 'text-slate-600'}`}>Habilitar Ganho Fixo?</button>
              </div>
              {newApp.isFixed && (
                <input className="w-full !bg-slate-900 !text-white !border-slate-800 text-center" type="number" placeholder="Valor Bruto (R$)" value={newApp.fixedVal} onChange={e => setNewApp({...newApp, fixedVal: e.target.value})} />
              )}
              ...
              }} className="w-full bg-blue-600/20 text-blue-500 font-black text-[10px] py-4 rounded-xl border border-blue-500/20 uppercase">Adicionar Perfil</button>
            </div>
            
            <button onClick={() => { if(!selectedApp) return alert("Crie/Selecione um App"); setShowAppConfig(false); setOdoInput(''); setShowOdo(true); }} className="w-full bg-blue-600 text-white py-6 rounded-[2rem] font-black text-lg uppercase italic shadow-2xl border-b-4 border-blue-800">Pronto para Trabalhar</button>
          </div>
        </div>
      )}

      {/* MODAL HODÔMETRO - REVISADO */}
      {showOdo && (
        <div className="fixed inset-0 bg-slate-950/98 z-[210] flex items-center justify-center p-6">
          <div className="w-full max-w-sm p-10 bg-slate-900 border border-white/10 rounded-[3rem] space-y-8 animate-up shadow-2xl text-center">
            <h2 className="text-2xl font-black uppercase italic text-white tracking-tighter">Confirmar KM Atual</h2>
            ...
            <button onClick={() => { 
              ...
              setShowOdo(false); 
            }} className="w-full bg-blue-600 text-white py-8 rounded-[2rem] font-black text-2xl uppercase italic border-b-8 border-blue-800 shadow-xl active:scale-95">Confirmar KM</button>
          </div>
        </div>
      )}

      {/* MODAL VALOR BRUTO - REVISADO */}
      {showGross && (
        <div className="fixed inset-0 bg-slate-950/98 z-[210] flex items-center justify-center p-6">
          <div className="w-full max-w-sm p-10 bg-slate-900 border border-white/10 rounded-[3rem] space-y-8 animate-up shadow-2xl text-center">
            <h2 className="text-2xl font-black uppercase italic text-white tracking-tighter">Valor Bruto da Viagem</h2>
            ...
            <button onClick={() => { 
              ...
              onFinishRace(val || 0);
              setShowGross(false); 
            }} className="w-full bg-emerald-600 text-white py-8 rounded-[2rem] font-black text-2xl uppercase italic border-b-8 border-emerald-800 shadow-xl active:scale-95">Salvar Ganho</button>
          </div>
        </div>
      )}
    </div>
